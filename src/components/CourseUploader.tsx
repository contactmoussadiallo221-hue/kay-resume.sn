...
export default function CourseUploader({
  ...
  const capturePhoto = () => {
    if (videoRef.current) {
      ...
      if (ctx) {
        ...
        setUploadedFile({
          name: `capture_resume_etude_${Date.now()}.jpg`,
          type: 'image/jpeg',
          data: dataUrl
        });
        stopCamera();
  ...
  return (
    <div className="premium-card p-5 premium-shadow sm:p-6 bg-white">
      
      <div className="mb-6 flex items-center justify-between border-b border-slate-50 pb-4">
        <h2 className="font-display text-base font-bold tracking-tight text-slate-900 flex items-center gap-2">
          <BookOpen className="h-5 w-5 text-indigo-600" />
          Saisir ou Importer votre Cours
        </h2>
        <span className="text-[10px] font-bold text-indigo-700 bg-indigo-50 px-2.5 py-1 rounded-full flex items-center gap-1 font-mono uppercase tracking-wider border border-indigo-100/40">
          <Sparkles className="h-3 w-3 text-indigo-500 fill-indigo-500" /> Résumé et Étude
        </span>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="space-y-4">
          <div>
            ...
            <select
              ...
              className="w-full rounded-xl border border-slate-200/80 bg-slate-50/50 px-3 py-2.5 text-xs font-semibold text-slate-700 focus:border-indigo-500 focus:bg-white focus:outline-none transition cursor-pointer"
            >
              <option value="Mathématiques">📐 Mathématiques</option>
              <option value="Physique-Chimie">🧪 Physique-Chimie</option>
              <option value="SVT / Biologie">🌿 SVT / Biologie</option>
              <option value="Histoire-Géographie">🌍 Histoire-Géographie</option>
              <option value="Français / Philosophie">✍️ Français / Philosophie</option>
          ...
          <div>
            <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5 flex items-center">
              Niveau d'explication désiré
            </label>
              ...
              {[
                ...
                <button
                  ...
                  className={`flex flex-col items-center justify-center rounded-xl p-2.5 border text-center transition-all ${
                    difficulty === level.id 
                    ? 'border-indigo-600 bg-indigo-50/50 text-indigo-700 ring-2 ring-indigo-600/15 font-semibold' 
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              className="w-full rounded-xl border border-slate-200/60 bg-slate-50/50 p-3.5 text-xs leading-relaxed text-slate-700 placeholder:text-slate-400 focus:border-indigo-500 focus:bg-white focus:outline-none transition resize-none font-sans"
            />
          </div>
        </div>
        ...
        <div className="flex flex-col">
          ...
          {useCamera ? (
            ...
            <div className="relative flex flex-1 flex-col items-center justify-center overflow-hidden rounded-xl bg-slate-950 border border-slate-900 min-h-[220px]">
              <video 
                ref={videoRef}
                playsInline
              ...
              <div className="absolute bottom-3 left-0 right-0 flex justify-center gap-2 px-3">
                <button
                  ...
                  className="rounded-xl bg-indigo-600 px-4 py-2 text-xs font-bold text-white shadow-lg shadow-indigo-600/30 hover:bg-indigo-500 transition"
                >
                  Capturer l'image
                </button>
                <button
            ...
            <div
              ...
              className={`flex flex-1 flex-col items-center justify-center rounded-xl border-2 border-dashed p-6 text-center cursor-pointer transition-all duration-200 ${
                dragActive 
            ...
            >
              ...
              {uploadedFile ? (
                ...
                <div className="space-y-3 w-full max-w-xs" onClick={(e) => e.stopPropagation()}>
                  <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-xl bg-indigo-50 border border-indigo-100 text-indigo-600">
                  ...
                  <div>
                    <p className="text-xs font-bold text-slate-800 truncate px-2" title={uploadedFile.name}>
                      {uploadedFile.name}
                    </p>
                    <p className="text-[10px] text-slate-400 font-bold font-mono mt-1 space-x-1.5 flex justify-center items-center">
                      <span className="bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded uppercase tracking-wide">
                        {uploadedFile.type.toUpperCase().split('/')[1] || "DOC"}
                      </span>
                      <span>•</span>
                      <button 
                ...
                <div className="space-y-3">
                  <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-slate-50 text-slate-400 border border-slate-100 group-hover:scale-105 transition-transform duration-200">
              ...
              {!uploadedFile && (
                <div className="mt-4" onClick={(e) => e.stopPropagation()}>
                  <button
                    ...
                    onClick={startCamera}
                    id="btn-lens-access"
                    className="inline-flex items-center gap-1.5 rounded-xl bg-slate-50 border border-slate-100 px-3 py-2 text-[11px] font-bold text-slate-700 hover:bg-indigo-50 hover:text-indigo-600 hover:border-indigo-100 transition duration-150"
                  >
      ...
      <div className="mt-6 border-t border-slate-100 pt-5 flex items-center justify-end">
        <button
          ...
          className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-tr from-indigo-700 to-indigo-600 px-6 py-3 text-xs font-extrabold text-white shadow-md shadow-indigo-600/10 hover:shadow-indigo-600/20 hover:from-indigo-600 hover:to-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-600/20 disabled:opacity-40 disabled:cursor-not-allowed transition duration-150 cursor-pointer"
        >
          {isProcessing ? (
            <>
            ...
            <>
              ...
              <span>Générer ma Fiche de Révision</span>
            </>
          )}
        </button>
      </div>
